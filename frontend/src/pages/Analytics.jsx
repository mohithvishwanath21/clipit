...
const Analytics = () => {
  ...
  return (
    <main className="min-h-screen bg-white text-gray-900 py-16 px-6 md:px-12 lg:px-24 max-w-5xl mx-auto">
      ...
      {/* Stats Grid */}
      <section className="grid grid-cols-2 sm:grid-cols-4 gap-6 max-w-4xl mx-auto mb-12">
        <StatCard icon={<Activity />} label="Today" value={data.today ?? 0} />
        <StatCard icon={<Calendar />} label="Yesterday" value={data.yesterday ?? 0} />
        <StatCard icon={<BarChart2 />} label="Last 7 Days" value={data.last7Days ?? 0} />
        <StatCard icon={<TrendingUp />} label="Last 30 Days" value={data.last30Days ?? 0} />
      </section>
    </main>
  );
};
