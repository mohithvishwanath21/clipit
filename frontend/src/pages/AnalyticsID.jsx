...
export const AnalyticsID = () => {
  ...
  const shortUrl = data?.shortId ? `${window.location.origin}/${data.shortId}` : "";

  console.log("Analytics Data:", data); // Check what you receive
  ...
  return (
    <main className="min-h-screen bg-white text-gray-900 py-16 px-6 md:px-12 lg:px-24 max-w-5xl mx-auto">
      ...
      <p className="text-center text-gray-500 mb-12">
        Breakdown of your shortened URL performance.
      </p>
      ...
      <section className="mb-12 max-w-3xl mx-auto">
        <div className="bg-white border border-red-200 rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="px-6 py-6 grid gap-4">
            {/* Original URL */}
            <div>
              <label className="text-xs font-semibold text-gray-500 block mb-1">Original URL</label>
              <div className="flex items-center gap-2">
                <a
                  href={data.originalUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-sm text-gray-800 underline underline-offset-2 break-all hover:text-red-600"
                >
                  {data.originalUrl}
                </a>
            ...
            {/* Shortened URL */}
            <div>
              <label className="text-xs font-semibold text-gray-500 block mb-1">Shortened URL</label>
              {shortUrl ? (
                <a
                  href={shortUrl}
                  target="_blank"
                ...
                <p className="text-sm text-red-400">Short URL not available</p>
              )}
            </div>
            ...
            {shortUrl && (
              <div className="mt-4">
                <label className="text-xs font-semibold text-gray-500 block mb-1">QR Code</label>
                <QRCodeCanvas
                  value={shortUrl}
                  size={100}
      ...
      {/* Stats Cards */}
      <section className="grid grid-cols-2 sm:grid-cols-3 gap-6 max-w-4xl mx-auto mb-12">
        <StatCard icon={<TrendingUp />} label="Last 7 Days" value={data.last7Days ?? 0} />
        <StatCard icon={<Calendar />} label="Last 30 Days" value={data.last30Days ?? 0} />
        <StatCard icon={<Activity />} label="Today" value={data.today ?? 0} />
      </section>

      {/* Status Card */}
      <section className="max-w-md mx-auto border border-gray-200 rounded-xl p-6 shadow-sm bg-white">
...
function StatCard({ icon, label, value }) {
  return (
    <div className="bg-white border border-red-100 rounded-md p-5 text-center shadow-sm hover:shadow-md transition">
      <div className="flex items-center justify-center mb-2">
        {React.cloneElement(icon, { className: "h-5 w-5 text-red-600" })}
      </div>
      <p className="text-2xl font-bold text-red-600">{value}</p>
      <p className="text-xs font-medium text-red-700 mt-1">{label}</p>
    </div>
  );
}
...
const Spinner = () => (
  <div className="flex justify-center items-center py-12 border border-gray-200 rounded-md">
    ...
    <span className="text-red-600 font-semibold ml-4">Loading...</span>
  </div>
);
