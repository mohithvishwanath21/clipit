...
import { deletedURL } from "../models/DELETION.js";
...
export const getSingleUrlAnalytics = async (req, res) => {
     console.log("✅ Backend hit: /analytics/:id");
  await connectDB();
  ...
  try {
    const url = await urlData.findOne(
      { _id: urlId, createdBy: userId },
      { originalUrl: 1, shortId: 1, isActive: 1 } // 🧠 only fetch what's needed
    );
...
export const updateUrlStatus = async (req, res) => {
  await connectDB();
  const { id } = req.params;
  ...
  try {
    const updated = await urlData.findOneAndUpdate(
      { _id: id },
      { isActive },
      { new: true }
    );
