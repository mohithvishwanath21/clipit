...
export const updateProfile = async (req, res) => {
  try {
    ...
    // Delete old avatar file if exists and is in uploads folder
    if (user.avatar && user.avatar.startsWith("/uploads/")) {
      const oldAvatarPath = path.resolve("." + user.avatar); // e.g. "./uploads/filename.png"
      if (fs.existsSync(oldAvatarPath)) {
        try {
          fs.unlinkSync(oldAvatarPath);
        } catch (unlinkErr) {
          console.error("Failed to delete old avatar:", unlinkErr);
          // Don't block operation, just log
        }
      }
    }
